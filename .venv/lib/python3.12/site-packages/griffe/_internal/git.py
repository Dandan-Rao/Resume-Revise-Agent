...
def tmp_worktree(repo: str | Path = ".", ref: str = "HEAD") -> Iterator[Path]:
    ...
    with TemporaryDirectory(
        prefix=f"{_WORKTREE_PREFIX}{repo_name}-{normref}-"
    ) as tmp_dir:
        location = os.path.join(tmp_dir, normref)  # noqa: PTH118
        tmp_branch = (
            f"griffe-{normref}"  # Temporary branch name must not already exist.
        )
        process = subprocess.run(
        ...
        if process.returncode:
            raise RuntimeError(
                f"Could not create git worktree: {process.stderr.decode()}"
            )
        ...
        try:
            ...
            subprocess.run(
                ["git", "-C", repo, "worktree", "remove", location],
                stdout=subprocess.DEVNULL,
                check=False,
            )
            subprocess.run(

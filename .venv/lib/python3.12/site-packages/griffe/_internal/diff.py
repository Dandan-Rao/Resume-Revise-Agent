...
_POSITIONAL_KEYWORD_ONLY = frozenset(
    (ParameterKind.positional_only, ParameterKind.keyword_only)
)
...
class Breakage:
    ...
    def __init__(
        self, obj: Object, old_value: Any, new_value: Any, details: str = ""
    ) -> None:
    ...
    def as_dict(
        self, *, full: bool = False, **kwargs: Any
    ...
    def _lineno(self) -> int:
        ...
        if (
            self.kind is BreakageKind.OBJECT_REMOVED
            and self._relative_filepath != self._location
        ):
            return 0
        if self.obj.is_alias:
    ...
    def _explain_oneline(self) -> str:
        explanation = (
            f"{self._format_location()}: {self._format_title()}: {self._format_kind()}"
        )
...
def _class_incompatibilities(
    ...
    *,
    ...
    yield from ()
    if new_class.bases != old_class.bases and len(new_class.bases) < len(
        old_class.bases
    ):
...
# TODO: Check decorators? Maybe resolved by extensions and/or dynamic analysis.
def _function_incompatibilities(
    ...
    for old_index, old_param in enumerate(old_function.parameters):
        ...
        if old_param.name not in new_function.parameters:
            swallowed = (
                ...
                or (
                    old_param.kind is ParameterKind.positional_only
                    and has_variadic_args
                )
                or (
                    old_param.kind is ParameterKind.positional_or_keyword
                    and has_variadic_args
                    and has_variadic_kwargs
                )
            )
            if not swallowed:
        ...
        if old_param.kind in _POSITIONAL and new_param.kind in _POSITIONAL:
            ...
            if new_index != old_index:
                ...
                yield ParameterMovedBreakage(
                    new_function, old_param, new_param, details=details
                )
        ...
        if old_param.kind is not new_param.kind:
            incompatible_kind = any(
                (
                    ...
                    old_param.kind is ParameterKind.positional_only
                    and new_param.kind is ParameterKind.keyword_only,
                    # Keyword-only to positional-only.
                    old_param.kind is ParameterKind.keyword_only
                    and new_param.kind is ParameterKind.positional_only,
        ...
        non_variadic = (
            old_param.kind not in _VARIADIC and new_param.kind not in _VARIADIC
        )
        if non_required and non_variadic:
            try:
                ...
                Exception
            ):  # noqa: BLE001 (equality checks sometimes fail, e.g. numpy arrays)
    ...
    if not _returns_are_compatible(old_function, new_function):
        yield ReturnChangedTypeBreakage(
            new_function, old_function.returns, new_function.returns
        )


def _attribute_incompatibilities(
    ...
    if old_attribute.value != new_attribute.value:
        if new_attribute.value is None:
            yield AttributeChangedValueBreakage(
                new_attribute, old_attribute.value, "unset"
            )
        else:
            yield AttributeChangedValueBreakage(
                new_attribute, old_attribute.value, new_attribute.value
            )
...
def _alias_incompatibilities(
    ...
    *,
    ...
    try:
        ...
        logger.debug(
            "API check: %s | %s: skip alias with unknown target",
            old_obj.path,
            new_obj.path,
        )
        return
    ...
    *,
    ...
    for name, old_member in old_obj.all_members.items():
        ...
        try:
            ...
            if (
                not old_member.is_alias and old_member.is_module
            ) or old_member.is_public:

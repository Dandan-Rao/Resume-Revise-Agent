    ...
    while curr_line_index < len(lines):
        ...
        for field_type in _field_types:
            if field_type.matches(line):
                ...
                curr_line_index = field_type.reader(
                    docstring, curr_line_index, parsed_values, **options
                )
                break
        else:
...
def _read_parameter(
    ...
    **options: Any,  # noqa: ARG001
    ...
    annotation = _determine_param_annotation(
        docstring, name, directive_type, parsed_values, warnings=warnings
    )
    ...
    **options: Any,  # noqa: ARG001
    ...
    if len(parsed_directive.directive_parts) == 2:  # noqa: PLR2004
        ...
        if warnings:
            docstring_warning(
                docstring,
                0,
                f"Failed to get parameter name from '{parsed_directive.line}'",
            )
        return parsed_directive.next_index
    ...
    if param is not None:
        if param.annotation is None:
            ...
            docstring_warning(
                docstring, 0, f"Duplicate parameter information for '{param_name}'"
            )
    return parsed_directive.next_index
    ...
    **options: Any,  # noqa: ARG001
    ...
    if len(parsed_directive.directive_parts) == 2:  # noqa: PLR2004
        ...
        if warnings:
            docstring_warning(
                docstring,
                0,
                f"Failed to get attribute name from '{parsed_directive.line}'",
            )
        return parsed_directive.next_index
    ...
    if attribute is not None:
        if attribute.annotation is None:
            ...
            docstring_warning(
                docstring, 0, f"Duplicate attribute information for '{attribute_name}'"
            )
    return parsed_directive.next_index
...
def _read_exception(
    ...
    **options: Any,  # noqa: ARG001
    ...
    if len(parsed_directive.directive_parts) == 2:  # noqa: PLR2004
        ...
        parsed_values.exceptions.append(
            DocstringRaise(annotation=ex_type, description=parsed_directive.value)
        )
    elif warnings:
        docstring_warning(
            docstring,
        ...
        try:
            ...
            if warnings and warn_missing_types:
                docstring_warning(
                    docstring,
                    0,
                    f"No return type or annotation at '{parsed_directive.line}'",
                )
            annotation = None

    # TODO: maybe support names
    parsed_values.return_value = DocstringReturn(
        name="", annotation=annotation, description=parsed_directive.value
    )
...
def _parse_directive(
    ...
    try:
        ...
        if warnings:
            docstring_warning(
                docstring, 0, f"Failed to get ':directive: value' pair from '{line}'"
            )

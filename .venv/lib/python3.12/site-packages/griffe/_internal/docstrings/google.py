...
_RE_ADMONITION: Pattern = re.compile(
    r"^(?P<type>[\w][\s\w-]*):(\s+(?P<title>[^\s].*))?\s*$", re.IGNORECASE
)
...
def _read_block(
    ...
    while new_offset < len(lines) and (
        lines[new_offset].startswith(indent * " ") or _is_empty_line(lines[new_offset])
    ):
    ...
    **options: Any,
    ...
    for line_number, param_lines in block:
        ...
        if warnings and warn_missing_types and annotation is None:
            docstring_warning(
                docstring, line_number, f"No type or annotation for parameter '{name}'"
            )
        ...
        if warnings and warn_unknown_params:
            with suppress(
                ...
                if name not in params:
                    message = (
                        f"Parameter '{name}' does not appear in the function signature"
                    )
        ...
        parameters.append(
            DocstringParameter(
                name=name, value=default, annotation=annotation, description=description
            )
        )
    ...
    **options: Any,
    ...
    for line_number, type_param_lines in block:
        ...
        if warn_unknown_params:
            with suppress(
                AttributeError
            ):  # for type parameters sections in objects without type parameters
...
def _read_functions_section(
    ...
    **options: Any,
    ...
    for line_number, func_lines in block:
        ...
        functions.append(
            DocstringFunction(name=name, annotation=signature, description=description)
        )
...
def _read_classes_section(
    ...
    **options: Any,
    ...
    for line_number, class_lines in block:
        ...
        classes.append(
            DocstringClass(name=name, annotation=signature, description=description)
        )
    ...
    for index, (line_number, return_lines) in enumerate(block):
        ...
        if annotation:
            ...
            annotation = _annotation_from_parent(
                docstring, gen_index=2, multiple=len(block) > 1, index=index
            )
    ...
    **options: Any,
    ...
    for index, (line_number, receive_lines) in enumerate(block):
        ...
        if annotation:
            ...
            # Try to retrieve the annotation from the docstring parent.
            annotation = _annotation_from_parent(
                docstring, gen_index=1, multiple=len(block) > 1, index=index
            )

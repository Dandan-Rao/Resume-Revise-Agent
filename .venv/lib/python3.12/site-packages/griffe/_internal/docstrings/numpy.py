    ...
    **options: Any,
    ...
    for item in items:
        ...
        if not match:
            if warnings:
                docstring_warning(
                    docstring, new_offset, f"Could not parse line '{item[0]}'"
                )
            continue
        ...
        if choices:
            ...
            match = re.match(
                r"^(?P<annotation>.+),\s+default(?: |: |=)(?P<default>.+)$", annotation
            )
            if match:
        ...
        if annotation is None:
            ...
            annotation = parse_docstring_annotation(
        ...
        parameters.extend(
            DocstringParameter(
                name, value=default, annotation=annotation, description=description
            )
            for name in names
        )
...
def _read_parameters_section(
    ...
    **options: Any,
    ...
    parameters, new_offset = _read_parameters(
        docstring, offset=offset, warnings=warnings, **options
    )
    if parameters:
        return DocstringSectionParameters(parameters), new_offset
    if warnings:
        docstring_warning(
            docstring, new_offset, f"Empty parameters section at line {offset}"
        )
...
def _read_other_parameters_section(
    ...
    **options: Any,
    ...
    if warnings:
        docstring_warning(
            docstring, new_offset, f"Empty other parameters section at line {offset}"
        )
    return None, new_offset
    ...
    **options: Any,
    ...
    for item in items:
        ...
        if choices:
            ...
            match = re.match(
                r"^(?P<annotation>.+),\s+default(?: |: |=)(?P<default>.+)$", bound
            )
            if match:
                default = match.group("default")
        ...
        if bound is None:
            ...
            bound = parse_docstring_annotation(
                bound, docstring, log_level=LogLevel.debug
            )
        ...
        type_parameters.extend(
            DocstringTypeParameter(
                name, value=default, annotation=bound, description=description
            )
            for name in names
        )
    ...
    docstring_warning(
        docstring, new_offset, f"Empty type parameters section at line {offset}"
    )
    return None, new_offset
...
def _read_deprecated_section(
    ...
    **options: Any,
    ...
    # SINCE_VERSION
    #     TEXT?
    items, new_offset = _read_block_items(
        docstring, offset=offset, warnings=warnings, **options
    )
    ...
    if warnings and len(items) > 1:
        docstring_warning(
            docstring, new_offset, f"Too many deprecated items at {offset}"
        )
...
def _read_returns_section(
    ...
    **options: Any,
    ...
    if not items:
        if warnings:
            docstring_warning(
                docstring, new_offset, f"Empty returns section at line {offset}"
            )
        return None, new_offset
    ...
    for index, item in enumerate(items):
        ...
        returns.append(
            DocstringReturn(name=name or "", annotation=annotation, description=text)
        )
...
def _read_yields_section(
    ...
    **options: Any,
    ...
    if not items:
        if warnings:
            docstring_warning(
                docstring, new_offset, f"Empty yields section at line {offset}"
            )
        return None, new_offset
    ...
    for index, item in enumerate(items):
        ...
        yields.append(
            DocstringYield(name=name or "", annotation=annotation, description=text)
        )
...
def _read_receives_section(
    ...
    **options: Any,
    ...
    for index, item in enumerate(items):
        ...
        receives.append(
            DocstringReceive(name=name or "", annotation=annotation, description=text)
        )
...
def _read_raises_section(
    ...
    **options: Any,
    ...
    if not items:
        if warnings:
            docstring_warning(
                docstring, new_offset, f"Empty raises section at line {offset}"
            )
        return None, new_offset
...
def _read_warns_section(
    ...
    **options: Any,
    ...
    if not items:
        if warnings:
            docstring_warning(
                docstring, new_offset, f"Empty warns section at line {offset}"
            )
        return None, new_offset
...
def _read_attributes_section(
    ...
    **options: Any,
    ...
    for item in items:
        ...
        text = dedent("\n".join(item[1:]))
        attributes.append(
            DocstringAttribute(name=name, annotation=annotation, description=text)
        )
...
def _read_functions_section(
    ...
    **options: Any,
    ...
    for item in items:
        ...
        text = dedent("\n".join(item[1:])).strip()
        functions.append(
            DocstringFunction(name=name, annotation=signature, description=text)
        )
...
def _read_classes_section(
    ...
    **options: Any,
    ...
    if not items:
        if warnings:
            docstring_warning(
                docstring, new_offset, f"Empty classes section at line {offset}"
            )
        return None, new_offset
...
def _read_type_aliases_section(
    ...
    **options: Any,
    ...
    if not items:
        docstring_warning(
            docstring, new_offset, f"Empty type aliases section at line {offset}"
        )
        return None, new_offset
...
def _read_modules_section(
    ...
    **options: Any,
    ...
    if not items:
        if warnings:
            docstring_warning(
                docstring, new_offset, f"Empty modules section at line {offset}"
            )
        return None, new_offset
...
def _read_examples_section(
    ...
    **options: Any,
    ...
    text, new_offset = _read_block(
        docstring, offset=offset, warnings=warnings, **options
    )

    sub_sections: list[
        tuple[Literal[DocstringSectionKind.text, DocstringSectionKind.examples], str]
    ] = []
    ...
    if warnings:
        docstring_warning(
            docstring, new_offset, f"Empty examples section at line {offset}"
        )
    return None, new_offset

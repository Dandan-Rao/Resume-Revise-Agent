...
class Decorator:
    ...
    def __init__(
        self, value: str | Expr, *, lineno: int | None, endlineno: int | None
    ) -> None:
...
class Docstring:
    ...
    def source(self) -> str:
        ...
        if isinstance(self.parent.filepath, list):
            raise ValueError(
                "Cannot get original docstring for namespace package"
            )  # noqa: TRY004
        if self.lineno is None or self.endlineno is None:
            raise ValueError("Cannot get original docstring without line numbers")
        return "\n".join(
            self.parent.lines_collection[self.parent.filepath][
                self.lineno - 1 : self.endlineno
            ]
        )
...
class Parameters:
    ...
    def __setitem__(self, name_or_index: int | str, parameter: Parameter) -> None:
        ...
        if isinstance(name_or_index, int):
            ...
            try:
                index = next(
                    idx for idx, param in enumerate(self._params) if param.name == name
                )
    ...
    def __contains__(self, param_name: str):
        ...
        try:
            next(
                param for param in self._params if param.name == param_name.lstrip("*")
            )
        except StopIteration:
            return False
        return True
...
class TypeParameters:
    ...
    def __setitem__(
        self, name_or_index: int | str, type_parameter: TypeParameter
    ) -> None:
    ...
    def __contains__(self, type_param_name: str):
        ...
        try:
            next(
                ...
                if param.name == type_param_name.lstrip("*")
            )
        except StopIteration:
            return False
        return True
...
class Object(ObjectAliasMixin):
    ...
    def has_docstrings(self) -> bool:
        ...
        for member in self.members.values():
            try:
                if (
                    not member.is_imported or member.is_public
    ...
    def is_kind(self, kind: str | Kind | set[str | Kind]) -> bool:
        ...
        if isinstance(kind, set):
            ...
            return self.kind in (
                knd if isinstance(knd, Kind) else Kind(knd) for knd in kind
            )
            ...
            for name, member in base.members.items():
                if name not in self.members:
                    inherited_members[name] = Alias(
                        name, member, parent=self, inherited=True
                    )
    ...
    def relative_filepath(self) -> Path:
        ...
        if isinstance(self.filepath, list):
            ...
            raise ValueError(
                f"No directory in {self.filepath!r} is relative to the current working directory {cwd}"
            )
        try:
    ...
    def resolve(self, name: str) -> str:
        ...
        if self.parent is None:
            raise NameResolutionError(
                f"{name} could not be resolved in the scope of {self.path}"
            )
    ...
    def as_dict(self, *, full: bool = False, **kwargs: Any) -> dict[str, Any]:
        ...
        if self.type_parameters:
            base["type_parameters"] = [
                type_param.as_dict(**kwargs) for type_param in self.type_parameters
            ]
        if self.labels:
            base["labels"] = self.labels
        if self.members:
            base["members"] = {
                name: member.as_dict(full=full, **kwargs)
                for name, member in self.members.items()
            }
...
class Alias(ObjectAliasMixin):
    ...
    def signature(self, *, return_type: bool = False, name: str | None = None) -> str:
        ...
        """
        return cast("Union[Class, Function]", self.final_target).signature(
...
class Module(Object):
    ...
    def __init__(
        self, *args: Any, filepath: Path | list[Path] | None = None, **kwargs: Any
    ) -> None:
    ...
    def resolved_bases(self) -> list[Object]:
        ...
        for base in self.bases:
            ...
            try:
                ...
                logger.debug(
                    "Base class %s is not loaded, or not static, it cannot be resolved",
                    base_path,
                )
            else:
...
class Function(Object):
    ...
    def signature(self, *, return_type: bool = True, name: str | None = None) -> str:
        ...
        has_pos_only = any(
            p.kind == ParameterKind.positional_only for p in self.parameters
        )

...
def _load_packages(
    ...
    *,
    ...
    for package in packages:
        ...
        try:
            loader.load(
                package, try_relative_path=True, find_stubs_package=find_stubs_package
            )
    ...
    if resolve_aliases:
        ...
        unresolved, iterations = loader.resolve_aliases(
            implicit=resolve_implicit, external=resolve_external
        )
        if unresolved:
            logger.info(
                "%s aliases were still unresolved after %s iterations",
                len(unresolved),
                iterations,
            )
        else:
...
def get_parser() -> argparse.ArgumentParser:
    ...
    main_help = (
        "Show this help message and exit. Commands also accept the -h/--help option."
    )
    ...
    def add_subparser(
        ...
        return subparsers.add_parser(
            command, add_help=False, help=text, description=text, **kwargs
        )

    # ========= DUMP PARSER ========= #
    dump_parser = add_subparser(
    ...
    dump_options.add_argument(
        ...
        metavar="PACKAGE",
        nargs="+",
        help="Packages to find, load and dump.",
    )
    dump_options.add_argument(
        "-f",
        "--full",
    ...
    *,
    ...
    if per_package_output:
        ...
        serialized = json.dumps(
            data_packages, cls=JSONEncoder, indent=2, full=full, sort_keys=True
        )

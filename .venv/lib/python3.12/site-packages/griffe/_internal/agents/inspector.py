...
class Inspector:
    ...
    def generic_inspect(self, node: ObjectNode) -> None:
        ...
        for child in node.children:
            if target_path := child.alias_target_path:
                ...
                if (
                    child.is_module
                    and target_path == f"{self.current.path}.{child.name}"
                ):
                    if not hasattr(child.obj, "__file__"):
                        logger.debug(
                            "Module %s is not discoverable on disk, inspecting right now",
                            target_path,
                        )
                        inspector = Inspector(
                            child.name,
        ...
        class_ = Class(
            ...
            type_parameters=TypeParameters(
                *_convert_type_parameters(
                    node.obj, parent=self.current, member=node.name
                )
            ),
            lineno=lineno,
            endlineno=endlineno,
    ...
    def handle_function(self, node: ObjectNode, labels: set | None = None) -> None:
        ...
        try:
            ...
            returns = (
                ...
                if return_annotation is _empty
                    ...
                    return_annotation, parent=self.current, member=node.name
                )
            )
        ...
        if obj.is_attribute:
            self.extensions.call(
                "on_attribute_instance", node=node, attr=obj, agent=self
            )
        else:
            self.extensions.call(
                "on_function_instance", node=node, func=obj, agent=self
            )
    ...
    def inspect_type_alias(self, node: ObjectNode) -> None:
        ...
        type_alias = TypeAlias(
            name=node.name,
            value=_convert_type_to_annotation(
                node.obj.__value__, parent=self.current, member=node.name
            ),
            lineno=lineno,
            endlineno=endlineno,
            type_parameters=TypeParameters(
    ...
    def handle_attribute(
        self, node: ObjectNode, annotation: str | Expr | None = None
    ) -> None:
        """Handle an attribute.
    ...
    *,
    ...
    annotation = (
        ...
        if parameter.annotation is _empty
            ...
            parameter.annotation, parent=parent, member=member
        )
    )
...
def _convert_object_to_annotation(
    ...
    if isinstance(obj, str):
        ...
        if hasattr(obj, "__name__"):  # noqa: SIM108
            ...
            annotation = obj_repr
    try:
        annotation_node = compile(
            annotation, mode="eval", filename="<>", flags=ast.PyCF_ONLY_AST, optimize=2
        )
    except SyntaxError:
        return obj
...
def _convert_type_to_annotation(
    ...
    args: Sequence[str | Expr | None] = [
        _convert_type_to_annotation(arg, parent=parent, member=member)

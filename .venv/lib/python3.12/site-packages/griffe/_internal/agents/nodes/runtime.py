...
def _same_components(a: str, b: str, /) -> bool:
    return [cpn.lstrip("_") for cpn in a.split(".")] == [
        cpn.lstrip("_") for cpn in b.split(".")
    ]
...
class ObjectNode:
    ...
    def is_function(self) -> bool:
        ...
        return inspect.isfunction(self.obj) or (
            callable(self.obj) and not self.is_class
        )
    ...
    def is_type_alias(self) -> bool:
        ...
        return isinstance(
            self.obj,
            tuple(
                module.TypeAliasType
                for module in _TYPING_MODULES
                if hasattr(module, "TypeAliasType")
            ),
        )
    ...
    def alias_target_path(self) -> str | None:
        ...
        child_name = getattr(
            self.obj, "__qualname__", self.path[len(self.module.path) + 1 :]
        )

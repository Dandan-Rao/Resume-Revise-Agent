...
class Visitor:
    ...
    if sys.version_info >= (3, 12):
        _type_parameter_kind_map: Final[
            dict[type[ast.type_param], TypeParameterKind]
        ] = {
    ...
    def visit_classdef(self, node: ast.ClassDef) -> None:
        ...
        # Handle base classes.
        bases = [
            safe_get_base_class(base, parent=self.current, member=node.name)
        ...
        class_ = Class(
            ...
            type_parameters=TypeParameters(
                *self._get_type_parameters(node, scope=node.name)
            ),
    ...
    def get_base_property(
        self, decorators: list[Decorator], function: Function
    ) -> str | None:
    ...
    def handle_function(
        self, node: ast.AsyncFunctionDef | ast.FunctionDef, labels: set | None = None
    ) -> None:
        ...
        if node.decorator_list:
            lineno = node.decorator_list[0].lineno
            for decorator_node in node.decorator_list:
                decorator_value = safe_get_expression(
                    decorator_node, parent=self.current, parse_strings=False
                )
        ...
        if "property" in labels:
            attribute = Attribute(
                name=node.name,
                value=None,
                annotation=safe_get_annotation(
                    node.returns, parent=self.current, member=node.name
                ),
                lineno=node.lineno,
                endlineno=node.end_lineno,
                docstring=self._get_docstring(node),
            ...
            self.extensions.call(
                "on_attribute_instance", node=node, attr=attribute, agent=self
            )
            return
        ...
        parameters = Parameters(
            *[
                Parameter(
                    ...
                    kind=kind,
                    annotation=safe_get_annotation(
                        annotation, parent=self.current, member=node.name
                    ),
                    default=(
                        default
                        if isinstance(default, str)
                        else safe_get_expression(
                            default, parent=self.current, parse_strings=False
                        )
                    ),
                )
        ...
        if overload:
            ...
            if (
                self.current.kind in {Kind.MODULE, Kind.CLASS}
                and self.current.overloads[function.name]
            ):
        ...
        self.extensions.call(
            "on_function_instance", node=node, func=function, agent=self
        )
    ...
    if sys.version_info >= (3, 12):
        ...
        def visit_typealias(self, node: ast.TypeAlias) -> None:
            ...
            type_alias = TypeAlias(
                ...
                endlineno=node.end_lineno,
                type_parameters=TypeParameters(
                    *self._get_type_parameters(node, scope=name)
                ),
                docstring=docstring,
                parent=self.current,
            )
            ...
            self.extensions.call(
                "on_type_alias_instance", node=node, type_alias=type_alias, agent=self
            )
    ...
    def visit_importfrom(self, node: ast.ImportFrom) -> None:
        ...
        for name in node.names:
            if (
                ...
                and node.level == 1
                and not name.asname
                and self.current.module.is_init_module
            ):
    ...
    def handle_attribute(
        ...
        value = safe_get_expression(
            node.value, parent=self.current, parse_strings=False
        )
                ...
                with suppress(AttributeError):
                    parent.exports = [
                        (
                            ...
                            else ExprName(name.name, parent=name.parent)
                        )
    ...
    def visit_annassign(self, node: ast.AnnAssign) -> None:
        ...
        self.handle_attribute(
            node, safe_get_annotation(node.annotation, parent=self.current)
        )
    ...
    def visit_if(self, node: ast.If) -> None:
        ...
        if isinstance(node.parent, (ast.Module, ast.ClassDef)):  # type: ignore[attr-defined]
            condition = safe_get_condition(
                node.test, parent=self.current, log_level=None
            )

...
class Extension:
    ...
    def on_node(
        self, *, node: ast.AST | ObjectNode, agent: Visitor | Inspector, **kwargs: Any
    ) -> None:
...
def _load_extension(
    ...
    # If the import path is a path to an existing file, load it.
        ...
        try:
            ...
            raise ExtensionNotLoadedError(
                f"Extension module '{import_path}' could not be found"
            ) from error
    ...
    if not ext_object:
        try:
            ...
            raise ExtensionNotLoadedError(
                f"Error while importing extension '{import_path}': {error}"
            ) from error

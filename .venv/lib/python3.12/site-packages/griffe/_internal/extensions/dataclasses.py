...
def _dataclass_decorator(decorators: list[Decorator]) -> Expr | None:
    for decorator in decorators:
        if (
            isinstance(decorator.value, Expr)
            and decorator.value.canonical_path == "dataclasses.dataclass"
        ):
...
def _expr_args(expr: Expr) -> dict[str, str | Expr]:
    ...
    if isinstance(expr, ExprCall):
        ...
        args.update(
            {
                ast.literal_eval(str(key)): value
...
def _dataclass_parameters(class_: Class) -> list[Parameter]:
    ...
    for member in class_.members.values():
        if member.is_attribute:
            ...
            if (
                isinstance(member.annotation, Expr)
                and member.annotation.canonical_path == "dataclasses.KW_ONLY"
            ):
                kw_only = True
                continue
            ...
            if "default_factory" in field_args:
                default: str | Expr | None = ExprCall(
                    function=field_args["default_factory"], arguments=[]
                )
            else:
                default = field_args.get(
                    "default", None if field_args else member.value
                )
...
def _del_members_annotated_as_initvar(class_: Class) -> None:
    ...
    attributes = [
        member for member in class_.members.values() if isinstance(member, Attribute)
    ]
    for attribute in attributes:
        if (
            isinstance(attribute.annotation, Expr)
            and attribute.annotation.canonical_path == "dataclasses.InitVar"
        ):

...
def _yield(
    ...
    *,
    ...
    if isinstance(element, Expr):
        ...
        for elem in element:
            yield from _yield(
                elem, flat=flat, outer_precedence=outer_precedence, is_left=is_left
            )
    else:
        yield element
...
def _join(
    ...
    *,
    ...
    try:
        ...
        yield from _yield(
            next(it), flat=flat, outer_precedence=_OperatorPrecedence.NONE
        )
...
class Expr:
    ...
    def canonical_name(self) -> str:
        ...
        return (
            self.canonical_path.rsplit(".", 1)[-1].split("(", 1)[-1].removesuffix(")")
        )
    ...
    def is_tuple(self) -> bool:
        ...
        return (
            isinstance(self, ExprSubscript) and self.canonical_name.lower() == "tuple"
        )
...
class ExprAttribute(Expr):
    ...
    def iterate(self, *, flat: bool = True) -> Iterator[str | Expr]:
        ...
        yield from _yield(
            self.values[0], flat=flat, outer_precedence=precedence, is_left=True
        )
...
class ExprBinOp(Expr):
    ...
    def iterate(self, *, flat: bool = True) -> Iterator[str | Expr]:
        ...
        yield from _yield(
            self.left, flat=flat, outer_precedence=precedence, is_left=True
        )
        yield f" {self.operator} "
        yield from _yield(
            self.right, flat=flat, outer_precedence=right_precedence, is_left=False
        )
...
class ExprBoolOp(Expr):
    ...
    def iterate(self, *, flat: bool = True) -> Iterator[str | Expr]:
        ...
        it = iter(self.values)
        yield from _yield(
            next(it), flat=flat, outer_precedence=precedence, is_left=True
        )
        for value in it:
            yield f" {self.operator} "
            yield from _yield(
                value, flat=flat, outer_precedence=precedence, is_left=False
            )
...
class ExprCall(Expr):
    ...
    def iterate(self, *, flat: bool = True) -> Iterator[str | Expr]:
        yield from _yield(
            self.function, flat=flat, outer_precedence=_get_precedence(self)
        )
        yield "("
...
class ExprFormatted(Expr):
    ...
    def iterate(self, *, flat: bool = True) -> Iterator[str | Expr]:
        ...
        yield from _yield(
            self.value, flat=flat, outer_precedence=_OperatorPrecedence.NONE
        )
        yield "}"
...
class ExprIfExp(Expr):
    ...
    def iterate(self, *, flat: bool = True) -> Iterator[str | Expr]:
        precedence = _get_precedence(self)
        yield from _yield(
            self.body, flat=flat, outer_precedence=precedence, is_left=True
        )
        yield " if "
        ...
        if isinstance(self.orelse, ExprIfExp):
            ...
            yield from _yield(
                self.orelse, flat=flat, outer_precedence=precedence, is_left=False
            )
    ...
    def iterate(self, *, flat: bool = True) -> Iterator[str | Expr]:
        ...
        for index, parameter in enumerate(self.parameters, 1):
            if parameter.kind is ParameterKind.positional_only:
                ...
                parameter.kind is ParameterKind.positional_or_keyword and not pos_or_kw
            ):
                pos_or_kw = True
            ...
            if parameter.default and parameter.kind not in (
        ...
        yield from _yield(
            self.body, flat=flat, outer_precedence=_OperatorPrecedence.NONE
        )
...
class ExprName(Expr):  # noqa: PLW1641
    ...
    def is_enum_class(self) -> bool:
        ...
        return any(
            isinstance(base, Expr) and base.canonical_path == "enum.Enum"
            for base in bases
        )
...
class ExprSubscript(Expr):
    ...
    def iterate(self, *, flat: bool = True) -> Iterator[str | Expr]:
        ...
        yield from _yield(
            self.slice, flat=flat, outer_precedence=_OperatorPrecedence.NONE
        )
        yield "]"
...
_precedence_map = {
    ...
    ExprUnaryOp: lambda e: {"not": _OperatorPrecedence.NOT}.get(
        e.operator, _OperatorPrecedence.POS_NEG_BIT_NOT
    ),
    ExprBinOp: lambda e: {
    ...
    ExprBoolOp: lambda e: {
        "and": _OperatorPrecedence.AND,
        "or": _OperatorPrecedence.OR,
    }[e.operator],
...
def _build_call(node: ast.Call, parent: Module | Class, **kwargs: Any) -> Expr:
    ...
    keyword_args = [
        _build(kwarg, parent, function=function, **kwargs) for kwarg in node.keywords
    ]
...
def _build_joinedstr(
    ...
    **kwargs: Any,
    ...
    return ExprJoinedStr(
        [_build(value, parent, in_joined_str=True, **kwargs) for value in node.values]
    )
...
def _build_keyword(
    ...
    **kwargs: Any,
    ...
    return ExprKeyword(
        node.arg, _build(node.value, parent, **kwargs), function=function
    )
        ...
        parameters=[
            ExprParameter(
                ...
                default=(
                    ...
                    else safe_get_expression(
                        default, parent=parent, parse_strings=False
                    )
                ),
            )
...
def _build_name(
    node: ast.Name, parent: Module | Class, member: str | None = None, **kwargs: Any
) -> Expr:  # noqa: ARG001
    return ExprName(node.id, parent, member)


def _build_named_expr(
    node: ast.NamedExpr, parent: Module | Class, **kwargs: Any
) -> Expr:
    return ExprNamedExpr(
        _build(node.target, parent, **kwargs), _build(node.value, parent, **kwargs)
    )
...
def _build_tuple(
    ...
    **kwargs: Any,
    ...
    return ExprTuple(
        [_build(el, parent, **kwargs) for el in node.elts], implicit=in_subscript
    )


def _build_unaryop(node: ast.UnaryOp, parent: Module | Class, **kwargs: Any) -> Expr:
    return ExprUnaryOp(
        _unary_op_map[type(node.op)], _build(node.operand, parent, **kwargs)
    )
...
def _build_yield_from(
    node: ast.YieldFrom, parent: Module | Class, **kwargs: Any
) -> Expr:
    ...
    *,
    ...
    try:
        ...
        message = msg_format.format(
            path=path, lineno=lineno, node_class=node_class, error=error_str
        )

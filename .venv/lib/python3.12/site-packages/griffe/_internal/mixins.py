        ...
        if len(parts) == 1:
            ...
            if name in self.members:  # type: ignore[attr-defined]
                ...
                if not member.is_alias:
                    ...
                    if member.is_module and not (
                        member.is_namespace_package or member.is_namespace_subpackage
                    ):
...
class SerializationMixin:
    ...
    def as_json(self, *, full: bool = False, **kwargs: Any) -> str:
        ...
        Returns:
            A JSON string.
        """
        from griffe._internal.encoders import (
            JSONEncoder,
        )  # Avoid circular import.  # noqa: PLC0415

        return json.dumps(self, cls=JSONEncoder, full=full, **kwargs)

    @classmethod
    def from_json(
        cls: type[_ObjType], json_string: str, **kwargs: Any
    ) -> _ObjType:  # noqa: PYI019
...
class ObjectAliasMixin(
    GetMembersMixin, SetMembersMixin, DelMembersMixin, SerializationMixin
):

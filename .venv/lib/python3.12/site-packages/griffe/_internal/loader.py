...
class GriffeLoader:
    ...
    def __init__(
        ...
        *,
        ...
        self.modules_collection: ModulesCollection = (
    ...
    def load(
        ...
        *,
        ...
        try:
            ...
            top_module_object = dynamic_import(
                top_module_name, self.finder.search_paths
            )
            ...
            try:
                ...
                if not top_module_path:
                    raise ValueError(
                        f"Module {top_module_name} has no paths set"
                    )  # noqa: TRY301
                ...
                logger.debug(
                    "Module %s has no paths set (built-in module?). Inspecting it as-is.",
                    top_module_name,
                )
            ...
            logger.debug(
                "Module %s has paths set: %s", top_module_name, top_module_path
            )
    ...
    def expand_exports(self, module: Module, seen: set | None = None) -> None:
        ...
        for export in module.exports:
            ...
            if isinstance(export, ExprName):
                module_path = export.canonical_path.rsplit(".", 1)[
                    0
                ]  # Remove trailing `.__all__`.
                try:
                    next_module = self.modules_collection.get_member(module_path)
                except KeyError:
                    logger.debug(
                        "Cannot expand '%s', try pre-loading corresponding package",
                ...
                try:
                    expanded += [
                        export
                        for export in next_module.exports
                        if export not in expanded
                    ]
                except TypeError:
                    logger.warning(
                        "Unsupported item in %s.__all__: %s (use strings only)",
                        module.path,
                        export,
                    )
        ...
        *,
        ...
        for member in obj.members.values():
            ...
            if member.is_alias and member.wildcard:  # type: ignore[union-attr]
                ...
                if not_loaded:
                    if external is False or (
                        external is None and package != f"_{obj.package.name}"
                    ):
                        continue
                    try:
                        ...
                        logger.debug(
                            "Could not expand wildcard import %s in %s: %s",
                            member.name,
                            obj.path,
                            error,
                        )
        ...
        for new_member, alias_lineno, alias_endlineno in expanded:
            ...
            self_alias = (
                new_member.is_alias
                and cast("Alias", new_member).target_path
                == f"{obj.path}.{new_member.name}"
            )
            ...
            if already_present:
                ...
                old_lineno = (
                    old_member.alias_lineno
                    if old_member.is_alias
                    else old_member.lineno
                )
            ...
            if member.is_alias:
                ...
                try:
                    ...
                    load_module = (
                        (
                            external is True
                            or (external is None and package == f"_{obj.package.name}")
                        )
                    ...
                    if load_module:
                        logger.debug(
                            "Failed to resolve alias %s -> %s", member.path, target
                        )
                        try:
                            self.load(package, try_relative_path=False)
                        except (ImportError, LoadingError) as error:
                            logger.debug(
                                "Could not follow alias %s: %s", member.path, error
                            )
    ...
    def _load_package(
        ...
        top_module = self._load_module(
            package.name, package.path, submodules=submodules
        )
        ...
        if package.stubs:
            ...
            stubs = self._load_module(
                package.name, package.stubs, submodules=submodules
            )
    ...
    def _load_module(
        ...
        *,
        ...
        try:
            return self._load_module_path(
                module_name, module_path, submodules=submodules, parent=parent
            )
            ...
            raise LoadingError(
                f"UnicodeDecodeError when loading {module_path}: {error}"
            ) from error
        except OSError as error:
            raise LoadingError(
                f"OSError when loading {module_path}: {error}"
            ) from error
    ...
    def _inspect_module(
        ...
        if self.store_source and filepath and filepath.suffix in {".py", ".pyi"}:
            self.lines_collection[filepath] = filepath.read_text(
                encoding="utf-8-sig"
        ...
        try:
            ...
            raise ImportError(
                f"Importing '{module_name}' raised a system exit"
            ) from error
        except Exception as error:
            raise ImportError(
                f"Importing '{module_name}' raised an exception"
            ) from error
        elapsed = datetime.now(tz=timezone.utc) - start
        ...
        for parent_offset, parent_part in enumerate(parent_parts, 2):
            ...
            try:
                ...
                if (
                    ...
                    or parent_module.is_namespace_subpackage
                ):
                    next_parent_module = self._create_module(
                        parent_part, [module_filepath]
                    )
                    parent_module.set_member(parent_part, next_parent_module)
                    parent_module = next_parent_module
                else:
                    raise UnimportableModuleError(
    ...
    ) -> list[tuple[Object | Alias, int | None, int | None]]:

...
def _simple_hostname_regex():
    ...
    return re.compile(
        r"^(?!-)[a-z0-9](?:[a-z0-9-]{0,59}[a-z0-9])?(?<!-)$", re.IGNORECASE
    )
    ...
    *,
    ...
    if may_have_port and (host_seg := _port_validator(value)):
        return (
            ...
            or (
                False if skip_ipv4_addr else ipv4(host_seg, cidr=False, private=private)
            )
    ...
    return (
        ...
        or domain(
            value, consider_tld=consider_tld, rfc_1034=rfc_1034, rfc_2782=rfc_2782
        )

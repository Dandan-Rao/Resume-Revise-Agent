...
class RunImpl:
        ...
        for output in response.output:
            if isinstance(output, ResponseOutputMessage):
                ...
                if output.server_label not in hosted_mcp_server_map:
                    ...
                    raise ModelBehaviorError(
                        f"MCP server label {output.server_label} not found"
                    )
                else:
                ...
                local_shell_calls.append(
                    ToolRunLocalShellCall(
                        tool_call=output, local_shell_tool=local_shell_tool
                    )
                )
                    ...
                    await asyncio.gather(
                        ...
                        (
                            agent.hooks.on_tool_end(
                                tool_context, agent, func_tool, result
                            )
                            if agent.hooks
                            else _coro.noop_coroutine()
                        ),
        ...
        *,
        ...
        for action in actions:
            ...
            if (
                ...
                and action.computer_tool.on_safety_check
            ):
                acknowledged = []
    ...
    async def execute_mcp_approval_requests(
        ...
        *,
        ...
        tasks = [
            run_single_approval(approval_request)
    ...
    async def execute(
        ...
        *,
        ...
        await asyncio.gather(
            ...
            (
                agent.hooks.on_tool_end(
                    context_wrapper, agent, call.local_shell_tool, result
                )
                if agent.hooks
                else _coro.noop_coroutine()
            ),

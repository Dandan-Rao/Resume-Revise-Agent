...
async def get_api_key(
    key: str | Callable[[], MaybeAwaitable[str]] | None,
) -> str | None:
...
class OpenAIRealtimeWebSocketModel(RealtimeModel):
    ...
    async def connect(self, options: RealtimeModelConfig) -> None:
        ...
        model_settings: RealtimeSessionModelSettings = options.get(
    ...
    async def _send_tracing_config(
        ...
        if tracing_config is not None:
            converted_tracing_config = _ConversionHelper.convert_tracing_config(
                tracing_config
            )
    ...
    async def _send_tool_output(self, event: RealtimeModelSendToolOutput) -> None:
        ...
        if event.start_response:
            await self._send_raw_message(
                OpenAIResponseCreateEvent(type="response.create")
            )
    ...
    def _get_playback_state(self) -> RealtimePlaybackState:
        ...
        if last_audio_item_id := self._audio_state_tracker.get_last_audio_item():
            ...
            audio_state = self._audio_state_tracker.get_state(
    ...
    async def _handle_audio_delta(self, parsed: ResponseAudioDeltaEvent) -> None:
        ...
        self._audio_state_tracker.on_audio_delta(
            parsed.item_id, parsed.content_index, audio_bytes
        )
        ...
        if item.type == "function_call" and item.status == "completed":
            ...
            message_item: RealtimeMessageItem = TypeAdapter(
            ...
            ).validate_python(
                {
                    ...
                    "role": item.role,
                    "content": (
                        [content.model_dump() for content in item.content]
                        if item.content
                        else []
                    ),
                    "status": "in_progress",
    ...
    async def _handle_ws_event(self, event: dict[str, Any]):
        ...
        if parsed.type == "response.audio.delta":
            ...
            await self._emit_event(
                RealtimeModelItemDeletedEvent(item_id=parsed.item_id)
            )
        elif (
    ...
    def _tools_to_session_tools(
        ...
        for tool in tools:
            if not isinstance(tool, FunctionTool):
                raise UserError(
                    f"Tool {tool.name} is unsupported. Must be a function tool."
                )
...
class _ConversionHelper:
    ...
    def convert_user_input_to_conversation_item(
        ...
        if isinstance(user_input, dict):
            ...
            return OpenAIConversationItem(
                ...
                role="user",
                content=[
                    OpenAIConversationItemContent(type="input_text", text=user_input)
                ],
            )
    ...
    def convert_tool_output(
        cls, event: RealtimeModelSendToolOutput
    ) -> OpenAIRealtimeClientEvent:
        return OpenAIConversationItemCreateEvent(
            type="conversation.item.create",

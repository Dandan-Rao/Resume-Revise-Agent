...
def realtime_handoff(
    ...
    *,
    ...
    is_enabled: (
        bool
        | Callable[[RunContextWrapper[Any], RealtimeAgent[Any]], MaybeAwaitable[bool]]
    ) = True,
...
def realtime_handoff(
    ...
    async def _is_enabled(
        ctx: RunContextWrapper[Any], agent_base: AgentBase[Any]
    ) -> bool:
        assert callable(is_enabled), "is_enabled must be non-null here"
        assert isinstance(
            agent_base, RealtimeAgent
        ), "Can't handoff to a non-RealtimeAgent"

...
class RealtimeSession(RealtimeModelListener):
    ...
    def __init__(
        ...
        self._debounce_text_length = self._run_config.get(
            "guardrails_settings", {}
        ).get("debounce_text_length", 100)
    ...
    async def __aenter__(self) -> RealtimeSession:
        ...
        model_config["initial_model_settings"] = (
            await self._get_updated_model_settings_from_agent(
                starting_settings=self._model_config.get(
    ...
    async def on_event(self, event: RealtimeModelEvent) -> None:
        ...
        if event.type == "error":
            await self._put_event(
                RealtimeError(info=self._event_info, error=event.error)
            )
            ...
            if current_length >= next_run_threshold:
                ...
                self._enqueue_guardrail_task(
                    self._item_transcripts[item_id], event.response_id
                )
        elif event.type == "item_updated":
            is_new = not any(
                item.item_id == event.item.item_id for item in self._history
            )
                        ...
                        if entry.type in ("audio", "input_audio"):
                            ...
                            if not entry_transcript:
                                ...
                                if idx < len(existing_content):
                                    ...
                                    if isinstance(
                                        this_content, AssistantAudio
                                    ) or isinstance(this_content, InputAudio):
                                ...
                                if not preserved and incoming_item.role == "assistant":
                                    preserved = self._item_transcripts.get(
                                        incoming_item.item_id
                                    )

                                if preserved:
                                    entry = entry.model_copy(
                                        update={"transcript": preserved}
                                    )
            ...
            if is_new:
                ...
                await self._put_event(
                    RealtimeHistoryAdded(info=self._event_info, item=new_item)
                )
            else:
                await self._put_event(
                    RealtimeHistoryUpdated(info=self._event_info, history=self._history)
            ...
            self._history = [
                item for item in self._history if item.item_id != deleted_id
            ]
            await self._put_event(
                RealtimeHistoryUpdated(info=self._event_info, history=self._history)
            )
    ...
    async def _handle_tool_call(self, event: RealtimeModelToolCallEvent) -> None:
        ...
        if event.name in function_map:
            ...
            result = await handoff.on_invoke_handoff(
                self._context_wrapper, event.arguments
            )
    ...
    def _get_new_history(
        ...
        existing_index = next(
            (i for i, item in enumerate(old_history) if item.item_id == event.item_id),
            None,
        )
        ...
        # Otherwise, insert it after the previous_item_id if that is set
            ...
            previous_index = next(
                (
                    i
                    for i, item in enumerate(old_history)
                    if item.item_id == event.previous_item_id
                ),
                None,
            )
    ...
    async def _run_output_guardrails(self, text: str, response_id: str) -> bool:
        ...
        combined_guardrails = (
            self._current_agent.output_guardrails
            + self._run_config.get("output_guardrails", [])
        )
        ...
        if triggered_results:
            ...
            guardrail_names = [
                result.guardrail.get_name() for result in triggered_results
            ]
            await self._model.send_event(
                RealtimeModelSendUserInput(
                ...
                asyncio.create_task(
                    self._put_event(
                        RealtimeError(
                            info=self._event_info,
                            error={
                                "message": f"Guardrail task failed: {str(exception)}"
                            },
                        )
                    )
                )
    ...
    async def _get_handoffs(
        ...
        async def _check_handoff_enabled(
            handoff_obj: Handoff[Any, RealtimeAgent[Any]],
        ) -> bool:

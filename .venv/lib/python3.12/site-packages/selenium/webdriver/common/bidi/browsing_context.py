...
class _EventManager:
    ...
    def __init__(self, conn, event_configs: dict[str, EventConfig]):
        ...
        self._bidi_to_class = {
            config.bidi_event: config.event_class for config in event_configs.values()
        }
    ...
    def validate_event(self, event: str) -> EventConfig:
        ...
        if not event_config:
            raise ValueError(
                f"Event '{event}' not found. Available events: {self._available_events}"
            )
        return event_config

    def subscribe_to_event(
        self, bidi_event: str, contexts: Optional[list[str]] = None
    ) -> None:
        ...
        with self._subscription_lock:
            if bidi_event not in self.subscriptions:
                ...
                self.conn.execute(
                    session.subscribe(bidi_event, browsing_contexts=contexts)
                )
...
class BrowsingContext:
    ...
    EVENT_CONFIGS = {
        "context_created": EventConfig(
            "context_created", "browsingContext.contextCreated", ContextCreated
        ),
        "context_destroyed": EventConfig(
            "context_destroyed", "browsingContext.contextDestroyed", ContextDestroyed
        ),
        "dom_content_loaded": EventConfig(
            "dom_content_loaded", "browsingContext.domContentLoaded", DomContentLoaded
        ),
        ...
        "fragment_navigated": EventConfig(
            "fragment_navigated", "browsingContext.fragmentNavigated", FragmentNavigated
        ),
        "history_updated": EventConfig(
            "history_updated", "browsingContext.historyUpdated", HistoryUpdated
        ),
        "load": EventConfig("load", "browsingContext.load", Load),
        "navigation_aborted": EventConfig(
            "navigation_aborted", "browsingContext.navigationAborted", NavigationAborted
        ),
        ...
        "navigation_failed": EventConfig(
            "navigation_failed", "browsingContext.navigationFailed", NavigationFailed
        ),
        "navigation_started": EventConfig(
            "navigation_started", "browsingContext.navigationStarted", NavigationStarted
        ),
        "user_prompt_closed": EventConfig(
            "user_prompt_closed", "browsingContext.userPromptClosed", UserPromptClosed
        ),
        "user_prompt_opened": EventConfig(
            "user_prompt_opened", "browsingContext.userPromptOpened", UserPromptOpened
        ),
    }
